...
namespace MLS.Application.DTO.Comment
{
    public class CreateCommentValidator : AbstractValidator<CreateCommentDto>
    {
        public CreateCommentValidator()
        {
            ...
            RuleFor(x => x.Author).MaximumLength(100).WithMessage("Author must be less than 100 characters.");
            RuleFor(x => x.Timestamp).LessThanOrEqualTo(DateTime.Now).WithMessage("Timestamp cannot be in the future.");
            RuleFor(x => x.ArticleId).GreaterThan(0).WithMessage("ArticleId must be greater than 0.");
